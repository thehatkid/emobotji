...
class CogCommandsManage(commands.Cog):
    ...
    async def cmd_delete(self, ctx: commands.Context, name: str):
        emoji = await self.db.get_emoji(name)
        if emoji is None:
            return await ctx.reply(f':x: Emoji `{name}` not found in bot.', mention_author=False)
        if ctx.author.id not in cfg['bot']['supervisors']:
            if ctx.author.id != emoji['author_id']:
        ...
        if emoji:
            ...
            await ctx.send(f':x: {name} was not found in bot\'s servers. Please contact to Bot Developer.')
